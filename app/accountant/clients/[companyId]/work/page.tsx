            ...
            return (
              <CollapsibleSection
                ...
                badge={tasks.filter(t => t.status !== 'completed' && t.status !== 'someday_maybe').length > 0 && (
                  <Badge variant="secondary" className="ml-2">{tasks.filter(t => t.status !== 'completed' && t.status !== 'someday_maybe').length}</Badge>
                )}
              >
            ...
            return (
              <CollapsibleSection
                ...
                badge={attention.unread_messages > 0 && (
                  <Badge variant="destructive" className="ml-2">{attention.unread_messages}</Badge>
                )}
              >
