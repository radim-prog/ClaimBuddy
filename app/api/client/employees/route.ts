...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!isStaff && !company.portal_sections?.employees) {
      return NextResponse.json({ error: 'Section not enabled' }, { status: 403 })
    }

    // Only return safe fields — NO personal_id, bank_account, salary
    const { data: employees, error } = await supabaseAdmin
      .from('employees')
      .select('id, first_name, last_name, position, contract_type, employment_start, employment_end, active')
    ...
    console.error('Client employees API error:', error)
