...
export async function GET(
  ...
  try {
    const links = await getLinksForEntity('project', params.id)
    return NextResponse.json(links)
  } catch (err) {
    console.error('Get project register documents error:', err)
...
export async function POST(
  ...
  try {
    ...
    if (document_ids.length > 100) {
      return NextResponse.json({ error: 'Max 100 documents at once' }, { status: 400 })
    }
    ...
    console.error('Link project documents error:', err)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    console.error('Unlink project document error:', err)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
