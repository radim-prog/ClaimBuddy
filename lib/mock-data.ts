...
// Původní mock companies (zachováno jako záloha)
const _legacyMockCompanies = [
  {
    id: 'company-1',
...
// Generovat monthly_closures pro každou AKTIVNÍ firmu × 12 měsíců aktuálního roku
// Všechno jako 'missing' - podklady se budou doplňovat manuálně při kontrole
...
// Pouze aktivní firmy mají closures
const activeCompanies = mockCompanies.filter((c: any) => c.status === 'active')

const activeMonths = months.slice(0, currentMonth + 1)

export type ClosureRecord = {
  id: string; company_id: string; period: string;
  status: string; bank_statement_status: string;
  expense_documents_status: string; income_invoices_status: string;
  vat_payable: number | null; income_tax_accrued: number | null;
  social_insurance: number | null; health_insurance: number | null;
  reminder_count: number; last_reminder_sent_at: string | null;
  notes: string | null; updated_by: string | null;
...
// DPH / VAT TRACKING
// ============================================
import type { VatReturn, VatReturnStatus } from '@/lib/types/vat'

// Generate VAT returns for all VAT payers for past months of current year
const vatPayerCompanies = activeCompanies.filter((c: any) => c.vat_payer)

export const mockVatReturns: VatReturn[] = vatPayerCompanies.flatMap((company: any) => {
  const now = new Date()
  ...
  for (let m = 1; m <= currentMonth; m++) {
    ...
    // DPH přiznání - all not_filed (demo)
    returns.push({
      id: `vat-dph-${company.id}-${period}`,
      company_id: company.id,
      period,
      type: 'dph',
      status: 'not_filed',
...
export function getVatReturnsByCompany(companyId: string): VatReturn[] {
  return mockVatReturns.filter(v => v.company_id === companyId)
}

export function getVatReturnsByPeriod(period: string): VatReturn[] {
  return mockVatReturns.filter(v => v.period === period)
}
