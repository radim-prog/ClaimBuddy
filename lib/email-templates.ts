/**
 * Email templates for Pojistná Pomoc
...
export function wrapInLayout(
  ...
  const unsubscribeBlock = options?.showUnsubscribe
    ? `<p style="margin: 8px 0 0; font-size: 12px; color: #9ca3af;">
         <a href="https://claims.zajcon.cz/unsubscribe" style="color: #9ca3af;">
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0"
          ...
          <tr>
            <td style="padding-top: 20px; text-align: center;
              ...
              <p style="margin: 0;">
                Tento email byl odeslán z Pojistná Pomoc •
                <a href="https://claims.zajcon.cz" style="color: #6b7280;">claims.zajcon.cz</a>
              </p>
              ${unsubscribeBlock}
            </td>
          </tr>
...
export function registrationVerification(name: string, verifyUrl: string): EmailTemplate {
  const subject = 'Ověřte svůj email — Pojistná Pomoc'

  const content = `
    <p style="margin: 0 0 16px; font-size: 16px; color: #111827;">Dobrý den, <strong>${name}</strong>,</p>
    <p style="margin: 0 0 16px; font-size: 15px; color: #374151; line-height: 1.6;">
      děkujeme za registraci v Pojistná Pomoc. Pro dokončení registrace prosím ověřte svou emailovou adresu kliknutím na tlačítko níže.
    </p>
...
děkujeme za registraci v Pojistná Pomoc. Pro dokončení registrace prosím ověřte svou emailovou adresu na tomto odkazu:
...
export function welcomeEmail(name: string, loginUrl: string): EmailTemplate {
  const subject = 'Vítejte v Pojistná Pomoc!'

  const content = `
    <p style="margin: 0 0 16px; font-size: 16px; color: #111827;">Dobrý den, <strong>${name}</strong>,</p>
    <p style="margin: 0 0 16px; font-size: 15px; color: #374151; line-height: 1.6;">
      váš účet v Pojistná Pomoc je aktivní. Jsme rádi, že vás máme na palubě.
    </p>
    <p style="margin: 0 0 8px; font-size: 15px; color: #374151;">S Pojistná Pomoc můžete:</p>
...
export function passwordReset(name: string, resetUrl: string, expiresIn: string): EmailTemplate {
  const subject = 'Obnovení hesla — Pojistná Pomoc'
...
export function newMessageNotification(
  ...
  const subject = `Nová zpráva od ${senderName} — Pojistná Pomoc`

  const content = `
    <p style="margin: 0 0 16px; font-size: 16px; color: #111827;">Dobrý den, <strong>${name}</strong>,</p>
    <p style="margin: 0 0 16px; font-size: 15px; color: #374151; line-height: 1.6;">
      máte novou zprávu od <strong>${senderName}</strong> v Pojistná Pomoc.
    </p>
    ...
    ${emailButton('Zobrazit zprávu', 'https://claims.zajcon.cz/komunikace')}`

  const text = `Dobrý den, ${name},

máte novou zprávu od ${senderName} v Pojistná Pomoc.

„${messagePreview}"

Zprávu zobrazíte zde: https://claims.zajcon.cz/komunikace
...
export function newDocumentInInbox(
  ...
  const subject = `Nový doklad v inboxu (${countLabel}) — Pojistná Pomoc`
  ...
  const content = `
    ...
    ${emailButton('Otevřít inbox', 'https://claims.zajcon.cz/inbox')}`
...
Inbox: https://claims.zajcon.cz/inbox
...
export function paymentReminder(
  ...
  const content = `
    ...
    ${emailButton('Zobrazit fakturu', 'https://claims.zajcon.cz/faktury', accentColor)}
    <p style="margin: 0; font-size: 13px; color: #6b7280;">
...
Fakturu zobrazíte zde: https://claims.zajcon.cz/faktury
...
export function paymentConfirmation(
  ...
  const content = `
    ...
    ${emailButton('Zobrazit přehled faktur', 'https://claims.zajcon.cz/faktury')}`
...
Přehled faktur: https://claims.zajcon.cz/faktury
...
export function invoiceIssued(
  ...
  const content = `
    ...
    ${emailButton('Zobrazit fakturu', 'https://claims.zajcon.cz/faktury')}`
...
export function deadlineApproaching(
  ...
  const content = `
    ...
    ${emailButton('Zobrazit termíny', 'https://claims.zajcon.cz/ukoly', '#d97706')}`
...
Termíny zobrazíte zde: https://claims.zajcon.cz/ukoly
...
export function marketplaceResponse(
  ...
  const statusBlock = accepted
    ? `<div style="background-color: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 6px;
       ...
       <p style="margin: 0 0 20px; font-size: 15px; color: #374151; line-height: 1.6;">
         <strong>${accountantName}</strong> přijala vaši žádost o spolupráci. Nyní ji můžete kontaktovat přímo v Pojistná Pomoc.
       </p>
       ${emailButton('Otevřít komunikaci', 'https://claims.zajcon.cz/komunikace')}`
    : `<div style="background-color: #fef2f2; border: 1px solid #fecaca; border-radius: 6px;
       ...
       ${emailButton('Najít jiného účetního', 'https://claims.zajcon.cz/marketplace')}`
  ...
  const textBody = accepted
    ? `${accountantName} přijala vaši žádost o spolupráci. Nyní ji můžete kontaktovat přímo v Pojistná Pomoc: https://claims.zajcon.cz/komunikace`
    : `Bohužel ${accountantName} v tuto chvíli nemůže přijmout nové klienty. V marketplace najdete další dostupné účetní: https://claims.zajcon.cz/marketplace`
...
export function trialEnding(name: string, daysLeft: number, planName: string): EmailTemplate {
  ...
  const content = `
    ...
    ${emailButton('Vybrat předplatné', 'https://claims.zajcon.cz/pricing', '#7c3aed')}
    <p style="margin: 0; font-size: 13px; color: #6b7280;">
...
Aby nedošlo k přerušení přístupu, aktivujte prosím předplatné na: https://claims.zajcon.cz/pricing
...
export function subscriptionChanged(
  ...
  const content = `
    ...
    ${emailButton('Správa předplatného', 'https://claims.zajcon.cz/settings')}`
...
Správa předplatného: https://claims.zajcon.cz/settings
...
export function taskAssigned(
  ...
  const content = `
    <p style="margin: 0 0 16px; font-size: 16px; color: #111827;">Dobrý den, <strong>${name}</strong>,</p>
    <p style="margin: 0 0 16px; font-size: 15px; color: #374151; line-height: 1.6;">
      <strong>${assignerName}</strong> vám přidělil nový úkol v Pojistná Pomoc.
    </p>
    ...
    ${emailButton('Otevřít úkol', 'https://claims.zajcon.cz/ukoly')}`

  const text = `Dobrý den, ${name},

${assignerName} vám přidělil nový úkol: ${taskTitle}

Úkol zobrazíte zde: https://claims.zajcon.cz/ukoly
...
export function questionnaireToFill(
  ...
  const content = `
    ...
    ${emailButton('Vyplnit dotazník', 'https://claims.zajcon.cz/dotazniky')}`
...
Dotazník vyplníte zde: https://claims.zajcon.cz/dotazniky
...
export function leadDeadlineReminder(name: string): EmailTemplate {
  ...
  const content = `
    ...
    <p style="margin: 0 0 20px; font-size: 15px; color: #374151; line-height: 1.6;">
      S účetním v Pojistná Pomoc vám žádný termín neuteče — systém hlídá vše automaticky.
    </p>
    ${emailButton('Najít účetního', 'https://claims.zajcon.cz/client/find-accountant')}
    `
...
S účetním v Pojistná Pomoc vám žádný termín neuteče.

Najít účetního: https://claims.zajcon.cz/client/find-accountant

Odhlásit se z emailů: https://claims.zajcon.cz/client/account
  ...
  const content = `
    ...
    <div style="margin-bottom: 20px;">
      ...
      <div style="border-left: 3px solid #2563eb; padding: 12px 16px; margin-bottom: 12px; background: #f9fafb; border-radius: 0 6px 6px 0;">
        ...
        <p style="margin: 0; font-size: 14px; color: #374151;">V Pojistná Pomoc máte sběrný email — přepošlete fakturu a je zařazena.</p>
      </div>
...
2. Posílejte doklady emailem — v Pojistná Pomoc máte sběrný email
3. Nechte AI vytěžit data — dodavatel, částka, DPH automaticky

Najít účetního: https://claims.zajcon.cz/client/find-accountant

Odhlásit se: https://claims.zajcon.cz/client/account
...
export function leadTaxSaving(name: string): EmailTemplate {
  ...
  const content = `
    ...
    <p style="margin: 0 0 20px; font-size: 15px; color: #374151; line-height: 1.6;">
      V Pojistná Pomoc vidíte svůj <strong>daňový dopad v reálném čase</strong>. S účetním ušetříte ještě víc.
    </p>
    ${emailButton('Najít účetního', 'https://claims.zajcon.cz/client/find-accountant')}
    `
...
export function leadAccountantOffer(name: string): EmailTemplate {
  ...
  const content = `
    <p style="margin: 0 0 16px; font-size: 16px; color: #111827;">Dobrý den, <strong>${name}</strong>,</p>
    <p style="margin: 0 0 16px; font-size: 15px; color: #374151; line-height: 1.6;">
      Podnikáte, ale účetnictví řešíte sami? Nemusíte. V Pojistná Pomoc najdete ověřené
    ...
    ${emailButton('Najít účetního zdarma', 'https://claims.zajcon.cz/client/find-accountant', '#16a34a')}
    `

  const text = `Dobrý den, ${name},

Podnikáte, ale účetnictví řešíte sami? V Pojistná Pomoc najdete ověřené účetní.
...
export function reportEmailTemplate(
  ...
  const content = `
    ...
    <p style="margin: 0; font-size: 12px; color: #9ca3af;">
      Tento report můžete vypnout v <a href="https://claims.zajcon.cz/accountant/settings" style="color: #9ca3af;">nastavení</a>.
    </p>`
...
Tento report můžete vypnout v nastavení: https://claims.zajcon.cz/accountant/settings
...
export function clientDataDeleted(
  ...
  const content = `
    ...
    ${emailButton('Zobrazit profil klienta', 'https://claims.zajcon.cz/klienti', '#dc2626')}`
...
Profil klienta: https://claims.zajcon.cz/klienti
