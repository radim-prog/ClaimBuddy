    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select
            ...
            onChange={e => setForm(f => ({ ...f, vehicle_category: e.target.value as VehicleCategory }))}
          >
        ...
        <div>
          <Label>Spotřeba z technického průkazu (l/100km) *</Label>
          ...
          <p className="text-xs text-muted-foreground mt-1">Kombinovaný provoz dle norem EU (z TP)</p>
        </div>
        <div>
        ...
        <div>
          <Label>Aktuální stav nádrže (l)</Label>
      ...
      <div className="rounded-lg bg-gray-50 dark:bg-gray-800/50 p-3 text-xs text-muted-foreground space-y-1">
        <div className="flex items-center gap-1.5 font-medium text-foreground mb-1">
          ...
          Zákonné sazby 2026 (vyhl. 573/2025 Sb.)
        </div>
        <p>Základní náhrada: <strong>{basicRate} Kč/km</strong> ({VEHICLE_CATEGORY_LABELS[form.vehicle_category]})</p>
        {fuelPrice && (
          <p>Průměrná cena PHM: <strong>{fuelPrice.price} {fuelPrice.unit}</strong> ({fuelPrice.label})</p>
        )}
        {form.fuel_consumption && (
          <p>
            Náhrada na 1 km: {basicRate} + {Number(form.fuel_consumption) / 100 * (fuelPrice?.price || 0) > 0
              ? `${(Number(form.fuel_consumption) / 100 * (fuelPrice?.price || 0)).toFixed(2)}`
              : '?'} = <strong>{(basicRate + Number(form.fuel_consumption) / 100 * (fuelPrice?.price || 0)).toFixed(2)} Kč/km</strong>
          </p>
        )}
      </div>
