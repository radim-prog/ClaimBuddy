...
export function KomunikaceSwimlaineTile({ category, conversations, maxItems = 5, onConversationClick, onShowAll }: SwimlaineTileProps) {
  ...
  return (
    <Card className={`rounded-xl shadow-soft border-0 overflow-hidden border-t-4 ${config.borderColor}`}>
      {/* Header */}
        ...
        {conversations.length === 0 ? (
          <div className="px-4 py-6 text-center">
            <p className="text-xs text-muted-foreground">
              {category === 'needs_response' ? 'Vse zodpovezeno' : category === 'completed' ? 'Zadne vyresene' : 'Zadne konverzace'}
            </p>
          </div>
        ) : (
          <>
            ...
            {remaining > 0 && (
              <button
                onClick={onShowAll}
                className="w-full px-4 py-2.5 text-xs font-medium text-purple-600 dark:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-900/10 transition-colors flex items-center justify-center gap-1"
              >
                Zobrazit vse ({conversations.length})
                <ChevronRight className="h-3 w-3" />
              </button>
            )}
            {remaining <= 0 && conversations.length > 0 && (
              <button
                onClick={onShowAll}
                className="w-full px-4 py-2 text-[10px] text-gray-400 hover:text-purple-600 dark:hover:text-purple-400 transition-colors flex items-center justify-center gap-1"
              >
                Zobrazit detail
                <ChevronRight className="h-3 w-3" />
              </button>
