...
function InlineRow({
  ...
  // Reset when data changes (month switch)
  useEffect(() => {
    const newVals = {
      revenue: data?.revenue ?? 0,
      expenses: data?.expenses ?? 0,
  ...
  const handleNumberChange = (field: keyof RowData, val: string) => {
    ...
    <tr className={`border-b border-gray-100 dark:border-gray-700/50 ${isQuarterlyInterim ? 'opacity-60' : ''}`}>
      ...
      <td className="px-2 py-2">
        <input
          ...
          onChange={e => handleNumberChange('vat_output', e.target.value)}
          className="w-full h-8 px-2 text-sm rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 focus:ring-2 focus:ring-purple-500 focus:border-transparent text-right"
          placeholder="0"
        />
      </td>
      <td className="px-2 py-2">
        <input
          ...
          onChange={e => handleNumberChange('vat_input', e.target.value)}
          className="w-full h-8 px-2 text-sm rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 focus:ring-2 focus:ring-purple-500 focus:border-transparent text-right"
          placeholder="0"
        />
      </td>
      <td className="px-3 py-2 text-sm font-semibold text-right whitespace-nowrap">
        <span className={vatResult > 0 ? 'text-red-600 dark:text-red-400' : vatResult < 0 ? 'text-green-600 dark:text-green-400' : 'text-gray-500'}>
          {vatResult > 0 ? '+' : ''}{formatCZK(vatResult)}
        </span>
      </td>
      <td className="px-2 py-2">
        <input
...
export function VatMatrix({ selectedYear }: { selectedYear: number }) {
  ...
  const getGroupSum = useCallback((companyIds: string[]) => {
    let revenue = 0, expenses = 0, vatOutput = 0, vatInput = 0
    ...
    return { revenue, expenses, vatOutput, vatInput, vatResult: vatOutput - vatInput }
  }, [taxDataMap, period])
    ...
    <>
      ...
      <div className="mb-4 flex items-center justify-center gap-3">
        <button
          ...
          className="p-2 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
        >
          <ChevronLeft className="h-5 w-5" />
        </button>
        <div className="px-6 py-2 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 font-bold rounded-lg text-center min-w-[180px]">
        ...
        <table className="w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gradient-to-r from-purple-600 to-blue-600">
            <tr>
              ...
              <th className="px-2 py-3 text-right text-xs font-medium text-white uppercase tracking-wider min-w-[110px]">DPH výst.</th>
              <th className="px-2 py-3 text-right text-xs font-medium text-white uppercase tracking-wider min-w-[110px]">DPH vstup</th>
              <th className="px-3 py-3 text-right text-xs font-medium text-white uppercase tracking-wider min-w-[100px]">DPH výsl.</th>
                            ...
                            <>
                              ...
                              <td className="px-3 py-2 text-right text-xs font-bold">
                                <span className={sum.vatResult > 0 ? 'text-red-600 dark:text-red-400' : sum.vatResult < 0 ? 'text-green-600 dark:text-green-400' : 'text-gray-500'}>
                                  Σ {sum.vatResult > 0 ? '+' : ''}{formatCZK(sum.vatResult)}
                                </span>
                              </td>
                            </>
                          ) : (
